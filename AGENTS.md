...
- **宿主专用键**：槽位版 `app_photo_uri_<slot>_<pkg>` / `app_video_uri_<slot>_<pkg>` 只供 UI 展示原始 URI，Hook 侧读的是 `app_remote_*`。另外全部 11 个 `theme_*` 键也会被全量推到远程组里（`save`/`syncAllToRemote` 不按键过滤），Hook 侧忽略。
